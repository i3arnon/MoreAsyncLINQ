...
static partial class MoreAsyncEnumerable
{
    ...
    public static IAsyncEnumerable<TResult> Split<TSource, TResult>(
        this IAsyncEnumerable<TSource> source,
        TSource separator,
        Func<IAsyncEnumerable<TSource>, CancellationToken, ValueTask<TResult>> resultSelector)
    {
        if (source is null) throw new ArgumentNullException(nameof(source));
        if (resultSelector is null) throw new ArgumentNullException(nameof(resultSelector));
        ...
        Func<IAsyncEnumerable<TSource>, CancellationToken, ValueTask<TResult>> resultSelector)
    {
        ...
        comparer ??= EqualityComparer<TSource>.Default;
        return source.Split(
            (element, _) => ValueTasks.FromResult(comparer.Equals(element, separator)),
            count,
            resultSelector);
    }
    ...
    public static IAsyncEnumerable<IAsyncEnumerable<TSource>> Split<TSource>(
        this IAsyncEnumerable<TSource> source,
        Func<TSource, CancellationToken, ValueTask<bool>> separatorFunc)
    {
        if (source is null) throw new ArgumentNullException(nameof(source));
        if (separatorFunc is null) throw new ArgumentNullException(nameof(separatorFunc));
        ...
        int count)
    {
        ...
        return source.Split(
            separatorFunc,
            count,
            (enumerable, _) => ValueTasks.FromResult(enumerable));
    }
        ...
        Func<IAsyncEnumerable<TSource>, CancellationToken, ValueTask<TResult>> resultSelector)
    {
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            ...
            await foreach (var element in source.WithCancellation(cancellationToken))
            {
                if (count > 0 && await separatorFunc(element, cancellationToken))
                {
                    yield return await resultSelector(items?.ToAsyncEnumerable() ?? AsyncEnumerable.Empty<TSource>(), cancellationToken);
            ...
            if (items is { Count: > 0 })
            {
                yield return await resultSelector(items.ToAsyncEnumerable(), cancellationToken);
            }
        }
    }
}