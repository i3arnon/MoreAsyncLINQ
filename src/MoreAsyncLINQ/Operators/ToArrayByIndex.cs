...
static partial class MoreAsyncEnumerable
{
    ...
    public static ValueTask<TSource[]> ToArrayByIndexAsync<TSource>(
        this IAsyncEnumerable<TSource> source,
        Func<TSource, CancellationToken, ValueTask<int>> indexSelector,
        CancellationToken cancellationToken = default)
    {
        if (source is null) throw new ArgumentNullException(nameof(source));
        if (indexSelector is null) throw new ArgumentNullException(nameof(indexSelector));
        ...
        return source.ToArrayByIndexAsync(
            indexSelector,
            static (element, _, _) => ValueTasks.FromResult(element),
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return source.ToArrayByIndexAsync(
            indexSelector,
            (element, _, cancellationToken) => resultSelector(element, cancellationToken),
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            await foreach (var element in source.WithCancellation(cancellationToken))
            {
                var index = await indexSelector(element, cancellationToken);
                if (index < 0)
                {
                    throw new IndexOutOfRangeException();
                }
            ...
            return await indexedElements.
                ...
                ToArrayByIndexAsync(
                    maxIndex + 1,
                    (tuple, _) => ValueTasks.FromResult(tuple.index),
                    (tuple, cancellationToken) => resultSelector(tuple.element, tuple.index, cancellationToken),
                    cancellationToken);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            await foreach (var element in source.WithCancellation(cancellationToken))
            {
                ...
                array[index] = await resultSelector(element, index, cancellationToken);
            }
